...
Npilot = 7000;          % Length of Pilot Symbols
...
% Extracting values from DSP Function
rxData_B = Bob.rxData;
rxData_B = rxData_B(:);
dataBits_B = dataBits;
h_est = Bob.h_est;
...
for ii=1:Nrho
    ...
    %% Angular Detector ( Constant Theta )

    theta_th=deg2rad(9);        % Adjust this as per the theta analysis from 3D Curve
    ...
    accepted_ang_B(ii)=sum(valid_ang);
    ...
    if any(valid_ang)
        ...
        ser_ang_B(ii) = mean(tx_B~=rx_B);
        SER_eve_ang(ii) = mean(tx_E~=rx_E);
        ...
        SecretFraction_ang(ii) = max(0,I_AB-I_AE);
    
        Pacc_ang(ii) = accepted_ang_B(ii)/Nsym;
    
        ESKR_ang_kbps(ii) = ...
            (fs/sps)*Pacc_ang(ii)*SecretFraction_ang(ii)/1000;
...
semilogy(rhoVec,Pacc_rad,'-o','LineWidth',1.5); hold on;
semilogy(rhoVec,Pacc_rect,'-s','LineWidth',1.5);
semilogy(rhoVec,Pacc_ang,'-^','LineWidth',1.5);

grid on;
xlabel('\rho');
ylabel('P_{sift}');

legend('Radial','Rectangular','Angular',...
    'Location','best');

title('Sifting Probability vs \rho');

% QSER ( SER for accepted symbols )
figure;

semilogy(rhoVec,ser_rad_B,'-o','LineWidth',1.5); hold on;
semilogy(rhoVec,ser_rect_B,'-s','LineWidth',1.5);
semilogy(rhoVec,ser_ang_B,'-^','LineWidth',1.5);

semilogy(rhoVec,SER_eve_rad,'--o','LineWidth',1.2);
semilogy(rhoVec,SER_eve_rect,'--s','LineWidth',1.2);
semilogy(rhoVec,SER_eve_ang,'--^','LineWidth',1.2);
...
title('Bob and Eve SER vs \rho');

% QBER 
figure;

semilogy(rhoVec,QBER_rad,'-o','LineWidth',1.5); hold on;
semilogy(rhoVec,QBER_rect,'-s','LineWidth',1.5);
semilogy(rhoVec,QBER_ang,'-^','LineWidth',1.5);

semilogy(rhoVec,BER_eve_rad,'--o','LineWidth',1.2);
semilogy(rhoVec,BER_eve_rect,'--s','LineWidth',1.2);
semilogy(rhoVec,BER_eve_ang,'--^','LineWidth',1.2);
...
title('Bob and Eve BER vs \rho');

% Secret Fraction
figure;

plot(rhoVec,SecretFraction_rad,'-o','LineWidth',1.5); hold on;
plot(rhoVec,SecretFraction_rect,'-s','LineWidth',1.5);
plot(rhoVec,SecretFraction_ang,'-^','LineWidth',1.5);

grid on;

xlabel('\rho');
ylabel('Secret Fraction');

legend('Radial','Rectangular','Angular',...
    'Location','best');

title('Secret Fraction vs \rho');

% ESKR
figure;

plot(rhoVec,ESKR_rad_kbps,'-o','LineWidth',1.5); hold on;
plot(rhoVec,ESKR_rect_kbps,'-s','LineWidth',1.5);
plot(rhoVec,ESKR_ang_kbps,'-^','LineWidth',1.5);
...
title('Estimated Secret Key Rate vs \rho');

% Accepted number of Symbols
figure;

plot(rhoVec,accepted_rad_B,'-o','LineWidth',1.5); hold on;
plot(rhoVec,accepted_rect_B,'-s','LineWidth',1.5);
plot(rhoVec,accepted_ang_B,'-^','LineWidth',1.5);

grid on;
xlabel('\rho');
ylabel('Accepted Symbols');

legend('Radial','Rectangular','Angular',...
    'Location','best');

title('Accepted Symbols vs \rho');
...
for ii=1:Nrho
    ...
    %% Angular Detector  ( Varying theta )

    theta_th=deg2rad(700*sigma_B*rho);      
    % This factor of 700 can be adjusted with respect to sigma so that,
    % almost all angles (0 - 35/40) gets covered for theta analysis

    r_min=sigma_B*1.5;         % Adjust this as per the rho analysis from 3D Curve 
